...
canvas.bind(
    "<Configure>",
    lambda e: canvas.itemconfig(canvas_window, width=e.width - 12)
)
list_frame.bind(
    "<Configure>",
...
def load():
    ...
    for rid, dt, txt, st in c.execute("""
        SELECT id, datetime, text, status
        FROM reminders
        ORDER BY
    ...
    """):
        ...
        label_status = "[PASSED]" if st == "passed" else "[PENDING]"
        Label(
            card,
            text=f"{label_status}  {dt}",
            anchor="w",
            bg="#f5f5f5"
        ).pack(fill=X)
        ...
        card.bind(
            "<Button-1>",
            lambda e, rid=rid, fr=card: select_card(rid, fr)
        )
        for w in card.winfo_children():
            w.bind(
                "<Button-1>",
...
def check_reminders():
    ...
    for r_id, dt_str, text in cur.fetchall():
        remind_time = datetime.strptime(dt_str, "%Y-%m-%d %H:%M")
        if now >= remind_time:
            linux_notify("Reminder", text)
            
            cur.execute(
                "UPDATE reminders SET status='passed' WHERE id=?",
                (r_id,)
            )
